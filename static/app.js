  ...
  if (araForm) {
    ...
    const kart = m => `
      <article class="kart${m.status === 'kapalı' ? ' kapali' : ''}">
        <a class="kart-kapak" href="${m.url}" style="--kr:${m.renk}" aria-hidden="true" tabindex="-1"><span class="kapak-ikon">${m.ikon}</span><span class="puan-rozet">${m.puan}</span>${m.status === 'kapalı' ? '<span class="etiket-kapali">Kapalı</span>' : ''}</a>
